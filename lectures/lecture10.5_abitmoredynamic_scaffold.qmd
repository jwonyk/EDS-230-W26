...
result1 <- diff1(initialC = 10, nx = 10, dx = 1, nt = 8, dt = 1, D = 0.5, area = 10)
result2 <- diff1(initialC = 10, nx = 10, dx = 1, nt = 8, dt = 2, D = 1, area = 10)
result3 <- diff1(initialC = 10, nx = 10, dx = 1, nt = 8, dt = 1, D = 1.25, area = 10)
...
filled.contour(result1$conc, xlab = "Time", ylab = "Distance")
filled.contour(result2$conc, xlab = "Time", ylab = "Distance")
filled.contour(result3$conc, xlab = "Time", ylab = "Distance")

# or if you prefer this orientation (Distance on x axis)
filled.contour(t(result1$conc), ylab = "Time", xlab = "Distance")
filled.contour(t(result2$conc), ylab = "Time", xlab = "Distance")
filled.contour(t(result3$conc), ylab = "Time", xlab = "Distance")

# you can also plot how much material is moving in to each cell at each time step
filled.contour(result1$qin, xlab = "Time", ylab = "Distance")
filled.contour(result2$qin, xlab = "Time", ylab = "Distance")
filled.contour(result3$qin, xlab = "Time", ylab = "Distance")
```
...
plot(resfast_fixtime$conc[ , 3],
     ylab = "Concentration for Location 3",
     xlab = "Time")
...
spatical_metrics = cbind.data.frame(cvar, cmean, time = seq(from = 1, to = 100))

# notice its the same as the number of time units (nt) used above
ggplot(spatical_metrics, aes(time, cvar)) +
  geom_line() +
